...
├─ board.html  board.js     내 결과 — 학생이 자기 팀 것만 (마감 전 현황 / 마감 후 내 순위·문항별 정오표)
├─ reveal.html reveal.js reveal.css  결과 발표 (운영자 전용, 관리키. 꼴찌 팀부터 한 문항씩 공개)
...
| `/api/status?code=` | GET (또는 POST `{팀코드}`) | 그 팀의 회차별 점수·남은 시도·다음 회차. 마감 후에는 그 팀의 `결과`(순위·최종점수·문항별 정오)도 함께 |
