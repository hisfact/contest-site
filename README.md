...
├─ board.html  board.js     내 결과 — 학생이 자기 팀 것만 (마감 전 현황 / 마감 후 점수·문항별 정오표, 순위 없음)
├─ reveal.html reveal.js reveal.css  결과 발표 (운영자 전용, 관리키. 상위 10팀만, 10위부터 한 문항씩 공개)
...
| `/api/status?code=` | GET (또는 POST `{팀코드}`) | 그 팀의 회차별 점수·남은 시도·다음 회차. 마감 후에는 그 팀의 `결과`(최종점수·문항별 정오·가짜 문항의 정답 풀이, 순위는 없음)도 함께 |
| `/api/board` | POST `{관리키}` | 전체 순위 (마감 전 진행 현황 / 마감 후 순위·문항별 통계). 관리키 없이는 401. 60초 캐시. 결과 발표 화면이 쓴다 |
